...
function showMainScreen() {
    ...
    controls.addEventListener("touchenter", (event) =>
        doTouchMove(event, true)
    );
    controls.addEventListener("touchleave", (event) =>
        doTouchMove(event, false)
    );
...
function buttonUp(button) {
    ...
    if (thing) {
        ...
        if (!sustaining) {
            ...
            sustainingNotes.push(
                CONSTANTS.LOWEST_PIANO_KEY_MIDI_NOTE + thing.note
            );
        }
    }
